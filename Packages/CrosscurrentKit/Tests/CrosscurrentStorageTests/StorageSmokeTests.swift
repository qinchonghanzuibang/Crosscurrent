...
@Test func mainAppMigratesAndAgentSharesCanonicalWrites() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentStorageTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let locations = DatabaseLocations(container: root)
    let mainDatabase = try CrosscurrentDatabase.open(at: locations, role: .mainApp)
    let main = CrosscurrentRepository(database: mainDatabase, writerInstance: "main-test")
    ...
    let agentDatabase = try CrosscurrentDatabase.open(at: locations, role: .agent)
    let agent = CrosscurrentRepository(database: agentDatabase, writerInstance: "agent-test")
...
@Test func durableLeaseExpiresAndCanBeTakenOver() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentLeaseTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let database = try CrosscurrentDatabase.open(at: DatabaseLocations(container: root), role: .mainApp)
    let firstWriter = CrosscurrentRepository(database: database, writerInstance: "writer-one")
    let secondWriter = CrosscurrentRepository(database: database, writerInstance: "writer-two")
...
@Test func foregroundLeaseTargetsTheRequestedJob() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentExactLeaseTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let database = try CrosscurrentDatabase.open(at: DatabaseLocations(container: root), role: .mainApp)
    let repository = CrosscurrentRepository(database: database, writerInstance: "foreground-test")
...
@Test func syncFailurePreservesLastSuccessfulRefresh() async throws {
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    let database = try CrosscurrentDatabase.open(at: DatabaseLocations(container: root), role: .mainApp)
    let repository = CrosscurrentRepository(database: database, writerInstance: "sync-test")
...
@Test func aiCacheAndConsentRemainPromptAndPolicyRevisionAware() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentAICacheTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let database = try CrosscurrentDatabase.open(at: DatabaseLocations(container: root), role: .mainApp)
    let repository = CrosscurrentRepository(database: database, writerInstance: "ai-cache-test")
...
@Test func agentRefusesAnUninitializedSchema() throws {
    ...
    #expect(throws: CrosscurrentStorageError.self) {
        _ = try CrosscurrentDatabase.open(at: locations, role: .agent)
    }
}

@Test func alreadyRunningAgentRefusesWritesAfterSchemaChanges() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentAgentSchemaPromotionTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let locations = DatabaseLocations(container: root)
    let mainDatabase = try CrosscurrentDatabase.open(at: locations, role: .mainApp)
    let agentDatabase = try CrosscurrentDatabase.open(at: locations, role: .agent)
    let agent = CrosscurrentRepository(database: agentDatabase, writerInstance: "old-agent")

    try await mainDatabase.pool.write { db in
        try db.execute(sql: "PRAGMA user_version = \(CrosscurrentDatabase.requiredSchemaVersion + 1)")
    }

    await #expect(throws: CrosscurrentStorageError.self) {
...
@Test func contentAddressedBlobUsesQuarantineBeforeDeletion() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentBlobTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let locations = DatabaseLocations(container: root)
    let database = try CrosscurrentDatabase.open(at: locations, role: .mainApp)
    let repository = CrosscurrentRepository(database: database, writerInstance: "blob-test")
...
@Test func policyPurgeRemovesContentWhileRemoteDeletionRetainsEvidence() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "CrosscurrentPurgeTests-\(UUID().uuidString)")
    defer { try? FileManager.default.removeItem(at: root) }
    let database = try CrosscurrentDatabase.open(at: DatabaseLocations(container: root), role: .mainApp)
    let repository = CrosscurrentRepository(database: database, writerInstance: "purge-test")
