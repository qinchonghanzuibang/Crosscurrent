...
public enum BundledPromptCatalog {
    public static let all: [BundledPrompt] = [
        ...
        BundledPrompt(task: .articleSummary, name: "Article summary", body: "Summarize this article in 2–4 complete sentences and no more than 140 words. Preserve important qualifications, cite supplied evidence labels, and never reproduce long passages.\n{{article}}", variables: ["article"]),
        BundledPrompt(task: .keyPoints, name: "Key points", body: "Return 4–8 distinct, meaningful bullet points backed by supplied evidence labels. Avoid trivial details, repetition, and long copied passages.\n{{article}}", variables: ["article"]),
