...
public final class CrosscurrentDatabase: @unchecked Sendable {
    ...
    public static func open(at locations: DatabaseLocations, role: RepositoryRole) throws -> CrosscurrentDatabase {
        if role == .mainApp {
            try locations.prepare()
        } else if !FileManager.default.fileExists(atPath: locations.canonicalDatabase.path) {
            throw CrosscurrentStorageError.databaseNotInitialized
        }
        ...
        return try MigrationFileLock.withLock(at: locations.migrationLock, mode: lockMode) {
            ...
            case .agent:
                guard foundVersion == requiredSchemaVersion else {
                    throw CrosscurrentStorageError.incompatibleSchema(found: foundVersion, required: requiredSchemaVersion)
                }
            case .mainApp:
                ...
                guard migratedVersion == requiredSchemaVersion else {
                    throw CrosscurrentStorageError.incompatibleSchema(found: migratedVersion, required: requiredSchemaVersion)
                }
                let integrity = try pool.read { db in try String.fetchOne(db, sql: "PRAGMA quick_check") }
                guard integrity == "ok" else {
                    throw CrosscurrentStorageError.integrityFailure(integrity ?? "unknown")
                }
            }

            return CrosscurrentDatabase(pool: pool, locations: locations, role: role)
        }
    }
    ...
    func withCanonicalWriteAccess<T>(_ operation: () throws -> T) throws -> T {
        try MigrationFileLock.withLock(at: locations.migrationLock, mode: .shared) {
            ...
            guard foundVersion == Self.requiredSchemaVersion else {
                throw CrosscurrentStorageError.incompatibleSchema(found: foundVersion, required: Self.requiredSchemaVersion)
            }
    ...
    private static func makePool(path: String) throws -> DatabasePool {
        ...
        configuration.label = "CrosscurrentCanonical"
    ...
    private static func createMigrationBackup(pool: DatabasePool, locations: DatabaseLocations, fromVersion: Int) throws {
        ...
        let backup = locations.backups.appending(path: "Crosscurrent-v\(fromVersion)-\(timestamp).sqlite")
