...
public actor BrowserCreatorSessionXPCClient: BrowserCreatorSessionClient {
    private let client: CrosscurrentXPCClient

    public init(teamID: String, signingMode: CCCodeSigningIdentity.SigningMode) {
        let peer = CCCodeSigningIdentity(teamID: teamID, bundleID: "com.chonghanqin.crosscurrent.browser", signingMode: signingMode)
        client = CrosscurrentXPCClient(machServiceName: "com.chonghanqin.crosscurrent.browser.control", expectedPeer: peer)
    }
