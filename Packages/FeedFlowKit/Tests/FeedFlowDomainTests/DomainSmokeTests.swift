...
@Test func shareInboxUsesAnAtomicLeaseAndTreatsCapturedSelectionAsUnknownPrivacy() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "FeedFlowShareInboxTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let locations = DatabaseLocations(container: root)
    let database = try FeedFlowDatabase.open(at: locations, role: .mainApp)
    let repository = FeedFlowRepository(database: database, writerInstance: "share-test")
...
@Test func shortChineseQueriesUseExplicitIndexesAndHistoryIsOptIn() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "FeedFlowSearchTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
...
@Test func vectorNamespacesUseDynamicDescriptorsAndPersistOutsideCanonicalStorage() async throws {
    let root = FileManager.default.temporaryDirectory.appending(path: "FeedFlowVectorTests-\(UUID().uuidString)", directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
