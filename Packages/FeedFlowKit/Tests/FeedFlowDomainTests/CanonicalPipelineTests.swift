...
private func makeRepository() throws -> (FeedFlowRepository, DatabaseLocations) {
    let root = FileManager.default.temporaryDirectory.appending(path: "FeedFlowPipelineTests-\(UUID().uuidString)")
    let locations = DatabaseLocations(container: root)
    let database = try FeedFlowDatabase.open(at: locations, role: .mainApp)
    return (FeedFlowRepository(database: database, writerInstance: "pipeline-tests"), locations)
}

private func seedSource(_ repository: FeedFlowRepository, sourceID: SourceID, endpointID: SourceEndpointID, name: String) async throws {
...
@discardableResult
private func seedItem(_ repository: FeedFlowRepository, sourceID: SourceID, endpointID: SourceEndpointID, externalID: String, title: String, text: String) async throws -> Item {
    let itemID = ItemID()
