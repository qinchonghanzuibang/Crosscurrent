...
public struct StatusPill: View {
    ...
    public init(_ text: String, color: Color = FeedFlowColor.accent) { self.text = text; self.color = color }
...
public struct SourceMonogram: View {
    ...
    public var body: some View {
        Text(String(name.prefix(1)).uppercased())
            ...
            .background(FeedFlowColor.accent.opacity(0.14), in: RoundedRectangle(cornerRadius: size * 0.3))
            .foregroundStyle(FeedFlowColor.accent)
    }
}
...
public struct EventReadMarker: View {
    ...
    public var body: some View {
        switch status {
        case .unread: Circle().fill(FeedFlowColor.accent).frame(width: 8, height: 8)
        case .updated: StatusPill(String(localized: "Updated"), color: FeedFlowColor.update)
