...
public struct ReaderWebView: NSViewRepresentable {
    ...
    public func makeNSView(context: Context) -> WKWebView {
        ...
        let world = WKContentWorld.world(name: "FeedFlowReaderSelection")
        configuration.userContentController.add(context.coordinator, contentWorld: world, name: "feedFlowSelection")
      ...
      document.addEventListener('selectionchange', () => {
        ...
        timer = setTimeout(() => {
          ...
          if (!selected || selected.rangeCount === 0 || selected.isCollapsed) {
            webkit.messageHandlers.feedFlowSelection.postMessage({});
            return;
          }
          ...
          webkit.messageHandlers.feedFlowSelection.postMessage({text, utf8Start:start, utf8Length:byteCount(text)});
        }, 80);
      });
    })();
    """#
