...
public enum BrowserCreatorDOMExtractor {
    ...
    public static func refresh(platform: AuthenticatedCreatorPlatform, cursor: ConnectorCursor?, in webView: WKWebView) async throws -> ConnectorRefreshPage {
        ...
        _ = try await webView.callAsyncJavaScript("window.scrollTo({top: document.documentElement.scrollHeight, behavior: 'instant'}); return true", arguments: [:], in: nil, contentWorld: .world(name: "FeedFlowCreator"))
    ...
    private static func snapshot(platform: AuthenticatedCreatorPlatform, in webView: WKWebView) async throws -> DOMSnapshot {
        ...
        let value = try await webView.callAsyncJavaScript("return (\(script))(platform)", arguments: ["platform": platform.rawValue], in: nil, contentWorld: .world(name: "FeedFlowCreator"))
