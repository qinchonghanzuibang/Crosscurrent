...
public actor FeedFlowRepository {
    public let role: RepositoryRole
    public let writerInstance: String

    private let database: FeedFlowDatabase
    private let observations: CrossProcessObservationHub
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    public init(database: FeedFlowDatabase, writerInstance: String = UUID().uuidString.lowercased(), observations: CrossProcessObservationHub = CrossProcessObservationHub()) {
    ...
    public func ensureSourceFolder(name: String, pathKey: String, parentID: UUID?, attributes: [String: String], sortOrder: Int, at date: Date = .now) throws -> UUID {
        ...
        _ = try mutate(domains: [.sources]) { db in
            ...
            else { throw FeedFlowStorageError.corruptRecord("SourceFolder") }
            result = uuid
        }
        guard let result else { throw FeedFlowStorageError.corruptRecord("SourceFolder") }
        return result
    }
    ...
    public func eventEvidence(eventID: EventID) throws -> [StoredEventEvidence] {
        try database.pool.read { db in
            try Row.fetchAll(
                ...
                else { throw FeedFlowStorageError.corruptRecord("Event evidence") }
        ...
        try database.pool.read { db in
            try Row.fetchAll(
                ...
                guard let id = Self.identifier(EventRevisionID.self, row["id"]), let kind = RevisionChangeKind(rawValue: row["change_kind"]) else {
                    throw FeedFlowStorageError.corruptRecord("Event revision history")
                }
    ...
    public func completeJob(_ lease: JobLease, checkpoint: Data? = nil, at date: Date = .now) throws -> Bool {
        try mutate(domains: [.jobs], idempotencyKey: "complete:\(lease.token.uuidString.lowercased())") { db in
            ...
            guard count == 1 else { throw FeedFlowStorageError.jobLeaseUnavailable }
            try db.execute(
    ...
    public func renewLease(_ lease: JobLease, duration: TimeInterval = 60, at date: Date = .now) throws -> JobLease {
        try mutateValue(domains: [.jobs]) { db in
            ...
            guard db.changesCount == 1 else { throw FeedFlowStorageError.jobLeaseUnavailable }
            var renewed = lease
    ...
    public func cancellationRequested(for lease: JobLease) throws -> Bool {
        try database.pool.read { db in
            ...
            guard valid == 1 else { throw FeedFlowStorageError.jobLeaseUnavailable }
    ...
    public func pendingEvidenceSegments(limit: Int = 250) throws -> [PendingEvidenceSegment] {
        try database.pool.read { db in
            ...
            return try rows.map { row in
                ...
                else { throw FeedFlowStorageError.corruptRecord("PendingEvidenceSegment") }
        ...
        try database.pool.read { db in
            ...
            return try eventRows.map { row in
                guard let eventID = Self.identifier(EventID.self, row["stable_event_id"]) else {
                    throw FeedFlowStorageError.corruptRecord("Event")
                }
    ...
    public func currentEventSnapshots(limit: Int = 500) throws -> [StoredEventSnapshot] {
        try database.pool.read { db in
            ...
            return try eventRows.map { row in
                guard let eventID = Self.identifier(EventID.self, row["stable_event_id"]) else { throw FeedFlowStorageError.corruptRecord("Event") }
                let revision = try Self.decodeEventRevision(row: row)
                let memberships = try Self.memberships(eventRevisionID: revision.id, db: db)
                ...
                else { throw FeedFlowStorageError.corruptRecord("Event primary membership") }
    ...
    public func digestState(briefingDay: Date) throws -> StoredDigestState? {
        try database.pool.read { db in
            ...
            else { throw FeedFlowStorageError.corruptRecord("Digest") }
    ...
    public func sourceSnapshots() throws -> [StoredSourceSnapshot] {
        try database.pool.read { db in
            try Row.fetchAll(
                ...
                else { throw FeedFlowStorageError.corruptRecord("Source") }
                ...
                let classification: SourceAIClassification? = try aiRow.map { value in
                    guard let id = UUID(uuidString: value["id"]), let access = AccessRequirement(rawValue: value["access_requirement"]), let privacy = ContentPrivacy(rawValue: value["content_privacy"]), let provenance = AssertionProvenance(rawValue: value["provenance"]) else { throw FeedFlowStorageError.corruptRecord("SourceAIClassification") }
                ...
                let coverage: SourceCoverageAssertion? = try coverageRow.map { value in
                    guard let id = UUID(uuidString: value["id"]), let ecosystem = CoverageEcosystem(rawValue: value["ecosystem"]), let provenance = AssertionProvenance(rawValue: value["provenance"]) else { throw FeedFlowStorageError.corruptRecord("SourceCoverageAssertion") }
    ...
    public func entitySnapshots() throws -> [StoredEntitySnapshot] {
        try database.pool.read { db in
            try Row.fetchAll(db, sql: "SELECT e.*, r.id AS revision_id, r.display_name, r.summary, r.created_at AS revision_created_at FROM entities e JOIN entity_revisions r ON r.id=e.current_revision_id ORDER BY r.display_name").map { row in
                guard let id = Self.identifier(EntityID.self, row["id"]), let revisionID = Self.identifier(EntityRevisionID.self, row["revision_id"]), let kind = EntityKind(rawValue: row["kind"]) else { throw FeedFlowStorageError.corruptRecord("Entity") }
                let aliases = try Row.fetchAll(db, sql: "SELECT * FROM entity_aliases WHERE entity_id=? ORDER BY value", arguments: [id.description]).map { alias -> EntityAlias in
                    guard let aliasID = Self.identifier(EntityAliasID.self, alias["id"]), let provenance = AssertionProvenance(rawValue: alias["provenance"]) else { throw FeedFlowStorageError.corruptRecord("EntityAlias") }
    ...
    public func topicSnapshots() throws -> [StoredTopicSnapshot] {
        try database.pool.read { db in
            try Row.fetchAll(db, sql: "SELECT t.*, r.id AS revision_id, r.name, r.summary, r.created_at, (SELECT COUNT(DISTINCT a.event_revision_id) FROM event_topic_assertions a WHERE a.topic_id=t.id) AS event_count FROM topics t JOIN topic_revisions r ON r.id=t.current_revision_id ORDER BY r.name").map { row in
                guard let id = Self.identifier(TopicID.self, row["id"]), let revisionID = Self.identifier(TopicRevisionID.self, row["revision_id"]) else { throw FeedFlowStorageError.corruptRecord("Topic") }
    ...
    public func itemState(endpointID: SourceEndpointID, externalID: String) throws -> StoredItemState? {
        try database.pool.read { db in
            ...
            else { throw FeedFlowStorageError.corruptRecord("Item") }
    ...
    public func itemSegments(revisionID: ItemRevisionID) throws -> [ItemSegment] {
        try database.pool.read { db in
            try Row.fetchAll(
                ...
                else { throw FeedFlowStorageError.corruptRecord("ItemSegment") }
                let hash: String = row["segment_hash"]
                return ItemSegment(
                    id: id,
                    lineageID: lineageID,
    ...
    public func sourceEndpoint(id: SourceEndpointID) throws -> SourceEndpoint? {
        try database.pool.read { db in
            ...
            else { throw FeedFlowStorageError.corruptRecord("SourceEndpoint") }
    ...
    public func activePrompt(for task: AITask) throws -> ActivePrompt? {
        try database.pool.read { db in
            ...
            else { throw FeedFlowStorageError.corruptRecord("PromptRevision") }
    ...
    private static func decodeEventRevision(row: Row) throws -> EventRevision {
        ...
        else { throw FeedFlowStorageError.corruptRecord("EventRevision") }
    ...
    private static func memberships(eventRevisionID: EventRevisionID, db: Database) throws -> [EventMembershipAssertion] {
        try Row.fetchAll(
            ...
            else { throw FeedFlowStorageError.corruptRecord("EventMembershipAssertion") }
    ...
    private static func decodeDigestRevision(row: Row, db: Database) throws -> DigestRevision {
        ...
        else { throw FeedFlowStorageError.corruptRecord("DigestRevision") }
        ...
        let entries = try entryRows.map { entry -> DigestEntry in
            ...
            else { throw FeedFlowStorageError.corruptRecord("DigestEntry") }
    ...
    private static func decodeJob(row: Row) throws -> DurableJob {
        ...
        else { throw FeedFlowStorageError.corruptRecord("DurableJob") }
        return DurableJob(
            id: id,
            kind: row["kind"],
