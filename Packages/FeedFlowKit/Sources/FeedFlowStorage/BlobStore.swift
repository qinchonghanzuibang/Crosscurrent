...
public actor CanonicalBlobStore {
    ...
    public init(locations: DatabaseLocations, repository: FeedFlowRepository, manager: FileManager = .default) {
    ...
    public func data(for blob: StoredBlob) throws -> Data {
        ...
        guard data.count == blob.byteCount, HTTPMetadataRedactor.digest(data) == blob.sha256 else {
            throw FeedFlowStorageError.invalidStagedData
        }
        return data
    }
}
...
public actor BlobGarbageCollector {
    ...
    public init(database: FeedFlowDatabase, manager: FileManager = .default, quarantineDuration: TimeInterval = 7 * 24 * 60 * 60) {
