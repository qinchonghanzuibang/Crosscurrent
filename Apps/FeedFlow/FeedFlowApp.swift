...
final class AppModel: ObservableObject {
    ...
    private(set) var repository: FeedFlowRepository?
    ...
    func start() async {
        ...
        do {
            ...
            let teamID = Bundle.main.object(forInfoDictionaryKey: "FeedFlowTeamIdentifier") as? String ?? ""
            if !teamID.isEmpty, let group = try? DatabaseLocations.appGroup() {
                ...
                let developmentContainer = Self.fixtureContainer ?? FileManager.default.temporaryDirectory
                    .appending(path: "FeedFlow-Development", directoryHint: .isDirectory)
                #else
            ...
            let repository = FeedFlowRepository(database: database, writerInstance: "main-\(UUID().uuidString.lowercased())")
            ...
            let browser = BrowserCreatorSessionXPCClient(teamID: teamID.isEmpty ? "TEAMID_REQUIRED" : teamID, signingMode: FFSigningEnvironment.currentMode)
            ...
            backgroundState = FeedFlowServices.agent.status.displayName
            browserWorkerState = FeedFlowServices.browser.status.displayName
    ...
    func foregroundRefresh(endpointID: SourceEndpointID) async throws {
        ...
        let job = DurableJob(kind: FeedFlowJobKind.refresh, inputHash: endpointID.description, idempotencyKey: "foreground-refresh:\(endpointID):\(UUID().uuidString.lowercased())", payload: payload)
        ...
        guard let (leasedJob, lease) = try await repository.leaseJob(id: job.id, owner: "main-foreground", duration: 120) else {
            throw FeedFlowStorageError.jobLeaseUnavailable
        }
        do {
    ...
    private func seedDevelopmentLibrary(_ repository: FeedFlowRepository) async throws {
...
enum FeedFlowServices {
    static var agent: SMAppService { .agent(plistName: "com.chonghanqin.feedflow.agent.plist") }
    static var browser: SMAppService { .agent(plistName: "com.chonghanqin.feedflow.browser.plist") }
}
