...
struct EventDetailView: View {
    ...
    private func header(_ event: EventCardModel) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack { StatusPill(event.readStatus == .updated ? String(localized: "Updated") : String(localized: "Event"), color: event.readStatus == .updated ? FeedFlowColor.update : FeedFlowColor.accent); Spacer(); Text(event.date, style: .relative).foregroundStyle(.secondary) }
                ...
                List(history) { revision in
                    HStack(alignment: .top, spacing: 12) {
                        Image(systemName: revision.id == event.revisionID ? "circle.fill" : "circle")
                            .foregroundStyle(revision.id == event.revisionID ? FeedFlowColor.accent : .secondary)
    ...
    var body: some View {
        HStack(spacing: 2) {
            ForEach(sections, id: \.self) { section in
                ...
                .background(selection == section ? FeedFlowColor.accent : Color.clear, in: RoundedRectangle(cornerRadius: 7))
