...
enum CrosscurrentBrowserWorkerMain {
    @MainActor static func main() {
        ...
        runtime.start()
        withExtendedLifetime(runtime) { application.run() }
    }
}
...
final class BrowserControlService: NSObject, CrosscurrentXPCService, @unchecked Sendable {
    ...
    private func handle(_ request: BrowserWorkerRequest) async throws -> BrowserWorkerResponse {
        ...
        case let .navigate(navigation):
            return try await owner.withPage(navigation) { webView in
        ...
        case let .authenticate(platform, accountID, presentsWindow):
            ...
            try await owner.presentLogin(profile: profile, url: BrowserCreatorDOMExtractor.loginURL(for: platform))
        ...
        case let .discover(discovery):
            ...
            return try await owner.withPage(navigation, profileName: discovery.platform.rawValue) { webView in
                .creatorIdentity(try await BrowserCreatorDOMExtractor.discover(platform: discovery.platform, in: webView))
            }
        case let .refresh(refresh):
            ...
            return try await owner.withPage(navigation, profileName: refresh.platform.rawValue) { webView in
                .refreshPage(try await BrowserCreatorDOMExtractor.refresh(platform: refresh.platform, cursor: refresh.cursor, in: webView))
            }
        case let .capture(capture):
            let navigation = BrowserNavigationRequest(profileID: capture.accountID.rawValue, url: capture.url)
            return try await owner.withPage(navigation, profileName: capture.platform.rawValue) { webView in
        ...
        case let .disconnect(_, accountID):
            try await owner.disconnect(profileID: accountID.rawValue)
            return .acknowledged
        }
    }
}
